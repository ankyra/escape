...
func (s *envSuite) Test_Status_Sets_Time(c *C) {
	now := time.Now()
	st := NewStatus(OK)
	c.Assert(st.UpdatedAt.Sub(now) > 1, Equals, true)
}

func (s *envSuite) Test_Status_IsOneOf(c *C) {
...
func (s *envSuite) Test_Status_IsError(c *C) {
...
func (s *envSuite) Test_Status_IsOK(c *C) {
	trueCases := []StatusCode{
		Empty,
		Pending,
		OK,
...
func (s *envSuite) Test_Status_IsRunning(c *C) {
	trueCases := []StatusCode{
		RunningPreStep,
		RunningMainStep,
		RunningPostStep,
