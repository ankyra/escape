...
func (p *Path) ScratchSpaceDirectory(metadata ReleaseMetadata) string {
	return filepath.Join(p.EscapeDirectory(), metadata.GetReleaseId())
}

func (p *Path) ScratchSpaceReleaseMetadata(metadata ReleaseMetadata) string {
...
func (p *Path) ReleaseLocation(metadata ReleaseMetadata) string {
...
func (p *Path) DependencyReleaseArchive(dependency Dependency) string {
	return filepath.Join(p.baseDir, dependency.GetReleaseId()+".tgz")
}

func (p *Path) DependencyDownloadTarget(dependency Dependency) string {
...
func (p *Path) UnpackedDepDirectoryReleaseMetadata(dependency Dependency) string {
...
func (p *Path) EnsureDependencyTypeDirectoryExists(dep Dependency) error {
	return util.MkdirRecursively(p.DepTypeDirectory(dep))
}

func (p *Path) EnsureScratchSpaceDirectoryExists(metadata ReleaseMetadata) error {
