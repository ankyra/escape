...
```bash
curl -O https://storage.googleapis.com/escape-releases-eu/escape-client/0.19.11/escape-v{{ version }}-linux-amd64.tgz
...
```bash
curl -O https://storage.googleapis.com/escape-releases-eu/escape-client/0.19.11/escape-v{{ version }}-linux-darwin.tgz
tar -xvzf escape-v{{version}}-linux-amd64.tgz
sudo mv escape /usr/bin/escape
```
